...
当前代码版本：**v2.0.1 · 运费联算、位置清除与清单直接保存**。运输业务基础边界见 [v2.0.0 交付说明](ITERATION-3-TRANSPORT.md)，最新修改见 [变更记录](CHANGELOG.md)，线上发布状态见 [Netlify/Supabase 部署说明](deploy/NETLIFY-SUPABASE.md)。
...
去返程底部均有“加入清单”和保存按钮，可选先预览，也可直接保存当前最新填写内容，无需先加入或重新加入。清单页返回会后台上传已加入的记录，先可靠记录请求再关闭弹层，不等待云端回执；正式账目仍在服务器确认后更新，下次登录随账号加载。底部“放弃修改”只删除本次本机草稿，不上传、不删除云端旧记录。空白或改回原样的表单可直接退出。使用返回键时，返程加入后又改动但未重新加入的字段、快记未加入的输入只留本机，不混入上传。

返程吨位、单价和实收可填两项补第三项，自动结果跟随来源修改；三项都已手动填写时保留差额和实收 0。清空实收可恢复按应收统计。装卸位置可“清除坐标”并保留文字，也可“清空位置”；清除会取消在途定位，已清除坐标不会从同一常用地点旧对象中重新带回。
