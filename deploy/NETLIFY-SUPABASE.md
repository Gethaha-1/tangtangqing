...
### v1.6.1 恢复验收（2026-09-05）

- 预发布部署 `6a9bef9df05889c6f0937eaa` 使用专用测试账号 B 验证，每份合成 v1/v2/v3 账本含 200 笔费用；保存分别约 3.39 / 3.33 / 3.30 秒，回读守恒与同 operationId 重放通过。耗时来自本次开发机到线上测试部署，不代替用户手机实测。
- 验收后通过原子 API 清理合成趟次和收支；测试账号 B 保留初始化目录供后续专用验收。已转正式使用的原测试账号 A 禁止作为写入验证目标。
- 下载目录中的两份真实 v1/v2 旧备份仅在本机临时 PostgreSQL 恢复，记录、金额和旧油费守恒通过；未上传这些备份到测试账号或覆盖真实云端账本。
- v1.6.1 不变更线上 schema，可通过恢复上一 Netlify 部署回滚应用；不得为回滚重置账本。
